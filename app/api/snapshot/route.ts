...
function computeMetrics(
  ...
  // By merchant
  const merchantMap = new Map<string, { total: number; count: number; categoryId: string; categoryName: string; sources: Set<string> }>();
  for (const t of expenses) {
    const e = merchantMap.get(t.merchant) ?? { total: 0, count: 0, categoryId: t.categoryId, categoryName: t.categoryName, sources: new Set<string>() };
